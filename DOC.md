...
- **Áreas / Zonas com Contorno Adaptativo Dinâmico (Remodelagem Geométrica em Tempo Real via SVG)**:
  ...
  - **Geometria Dinâmica Adaptativa em Formato de Bolha (SVG Bubble / Convex Hull com Cantos Arredondados + Medição Real DOM)**:
    ...
    - **Cabeçalho Flutuante com Arrastar em Bloco**: O badge de identificação da área (`[ 🏢 ADM 4º ANDAR (2 itens) ]`) flutua dinamicamente ancorado logo acima do equipamento mais alto do bloco. Ao arrastar por esse cabeçalho, todos os nós membros se movem em sincronia pelo canvas.
