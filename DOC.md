...
20. **Fit Tela Automático na Abertura Direta (Cold Load) e Carrossel de Fluxogramas (TV NOC)**:
    - **Enquadramento Inteligente em Qualquer Cenário de Acesso**: O Fit Tela automático é disparado e calibrado tanto na **abertura direta via URL** (`/noc?view=map&map_id=X`, `/noc?map_id=X`, `/admin/zabbix`) quanto na alternância contínua do **Carrossel da TV**, sem depender de intervenção manual ou de início prévio da playlist.
    ...
    - **Ciclo Pós-Renderização com `ResizeObserver` e Micro-Ticks**: Implementado `useEffect` dedicado que aguarda a montagem e renderização real dos cartões no DOM (`loading = false`) combinando `requestAnimationFrame` e retentativas escalonadas (60ms, 180ms e 400ms), além de um `ResizeObserver` no container do canvas para recalibrar instantaneamente o zoom e o pan caso o container receba suas dimensões definitivas após o layout CSS da página.
    - **Supressão de Corrida de Sincronização**: Eliminação do disparo concorrente de `refreshTrigger` na montagem inicial, prevenindo que recargas secundárias de métricas em segundo plano interfiram no auto-fit inicial.
