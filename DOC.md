...
- **Áreas / Zonas com Contorno Adaptativo Dinâmico (Remodelagem Geométrica em Tempo Real via SVG)**:
  ...
  - **Geometria Dinâmica Adaptativa em Formato de Bolha (SVG Bubble / Convex Hull com Cantos Arredondados + Medição Real DOM)**:
    ...
    - **Movimento e Arraste Fluido de Áreas e Equipamentos**:
      ...
      - **Arraste de Área por Qualquer Ponto do Contorno**: A área pode ser movida pelo seu cabeçalho flutuante OU clicando e arrastando diretamente sobre a linha tracejada da bolha SVG.
      - **Persistência Automática ao Soltar (MouseUp)**: Toda movimentação de nós ou áreas é gravada de imediato no backend no momento em que o operador solta o botão do mouse, impedindo que posições sejam resetadas por timers de polling ou sincronização.
      - **Botão de Auto-Organização em Grade (`[ ⊞ ]`)**: Cada cabeçalho de área possui um botão direto para redistribuir todos os seus equipamentos em grade alinhada (3 a 4 colunas) com espaçamento perfeito de 1 clique.
